...
class TestLockfileDiscovery:
    """Which dependency files `minireg audit` will find.

    A project that keeps pip-compile output next to a ranged
    `requirements.txt` is the common shape, and it is the shape this repo
    uses. Missing the lock meant the audit found only the ranged file,
    correctly reported that ranges cannot be audited, and exited clean --
    having checked nothing at all.
    """

    def test_pip_compile_lock_names_are_recognised(self):
        names = [name for name, _eco, _parser in cli.LOCKFILES]
        assert "requirements.lock" in names
        assert "requirements-lock.txt" in names

    def test_a_lock_is_reported_before_the_ranged_file(self):
        names = [name for name, _eco, _parser in cli.LOCKFILES]
        assert names.index("requirements.lock") < names.index("requirements.txt")

    def test_hash_pinned_lines_parse(self, tmp_path):
        """pip-compile writes the pin and its hashes across continuation
        lines; the hashes must not be mistaken for requirements."""
        ...
        parsed = cli.parse_requirements(lock)
        assert parsed == [
            {"name": "aiofiles", "version": "25.1.0"},
            {"name": "fastapi", "version": "0.141.1"},
        ]

    def test_this_repos_own_lockfile_is_discovered(self):
        """Guards the CI job that audits this project against itself."""
        import pathlib
